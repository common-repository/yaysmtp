...
if ( ZohoServiceVendController::isDiffInfo() || ZohoServiceVendController::isExpired() ) {
	$errorV = 'No access token. Please check your client secret or re-confirm authorization';
	// LogErrors::clearErr();
  ...
  <div class="setting-el">
	...
	<div class="setting-el">
	  ...
	  <?php
		...
		<a class="yay-smtp-button-secondary" href="
				<?php
				if ( Utils::getCurrentMailer() !== 'zoho' ) {
					echo '#';
				} else {
					echo wp_kses_post( ZohoServiceVendController::generate_auth_code_url() );
				}
				?>
  " >Confirm authorization</a>
	  ...
	  <?php } else { ?>
		...
		<a href="#" class="yay-smtp-button-secondary yaysmtp-yoho-remove-auth">
					<?php echo esc_html__( 'Remove Authorization', 'yay-smtp' ); ?>
		</a>
	  ...
	  <?php } ?>
			...
			<?php
			if ( '' !== $errorV ) {
				echo '<p class="inline-error">' . wp_kses_post( $errorV ) . '</p>';
			}
			?>
